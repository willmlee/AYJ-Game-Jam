...
function create() {
  ...
  // When gameState.active is false, the game will listen for a pointerup event and restart when the event happens  
   this.input.on('pointerup', () => {
     if (gameState.active === false && numOfTotalEnemies() > 0) {
       this.scene.restart();
    } else if(gameState.active === false && numOfTotalEnemies() === 0) {
  ...
  this.physics.add.collider(pellets, gameState.player, () => {
    ...
    gameState.enemyVelocity = 1;
     this.add.text(150, 150, 'Game Over', { fontSize: '30px', fill: '#000', strokeThickness: '5' });
     this.add.text(90, 200, 'Click to Restart', { fontSize: '30px', fill: '#000', strokeThickness: '5' });
  });
...
function update() {
  if (gameState.active) {
    ...
    if (Phaser.Input.Keyboard.JustDown(gameState.cursors.space)) {
      gameState.bugRepellent.create(gameState.player.x, gameState.player.y, 'bugRepellent').setGravityY(-5000);
    }
    ...
    if (numOfTotalEnemies() === 0) {
      ...
      this.physics.pause();
      gameState.enemyVelocity = 1;
      this.add.text(105, 150, 'You Have Won!', { fontSize: '30px', fill: '#000000', strokeThickness: '5'});
      this.add.text(40, 200, 'Click to Start Next Level', { fontSize: '30px', fill: '#000000', strokeThickness: '5'});
      /*this.input.on('pointerup', () => {
        this.scene.stop('scene1');
        this.scene.start('scene2');
      }) */
